...
public class  EasyJWT {
    ...
    public String verify(String token) throws JWTVerificationException {
        Algorithm algorithm = Algorithm.HMAC256(certificationProperties.getSecret());
        JWTVerifier jwtVerifier = JWT.require(algorithm).withIssuer(new String[]{certificationProperties.getIssuer()}).build();
        return jwtVerifier.verify(token).getClaim("payload").asString();
    }
}
